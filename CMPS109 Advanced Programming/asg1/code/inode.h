...
class inode_state {
   ...
   public:
      inode_state();
      void init(inode* root_node);
      void cat (const wordvec& words);
      void cd();
      void cd_dir(const wordvec& words);
      void ls(const wordvec& words);
      void lsr(inode_state& state, const wordvec& words);
      void mk(const wordvec& words);
      void mkdir(const wordvec& words);
      void pwd();
      void rm(const wordvec& words);
      void rmr(inode_state& state, const wordvec& words);
      void change_prompt(const wordvec& words);
      string get_prompt();
};
...
class inode {
   ...
   public:
      ...
      void write_file(const wordvec& words);
      void writefile (const wordvec& newdata);
      void remove (const string& filename);
      inode& mkdir (const string& dirname);
      inode& mkfile (const string& filename);
      void ls_r(inode_state& state, const wordvec& words);
      void rm_r(inode_state& state, const wordvec& words,
