...
void sortedInsert(ListRef L, int data){
...
//enqueue method for sake of BFS and treating a list as a queue
void enqueue(ListRef Q, int data){
	insertAfterLast(Q, data);
}
//dequeue for sake of BFS when using list as queue
int dequeue(ListRef Q){
	int value = getFirst(Q);
	deleteFirst(Q);
	return value;
}
...
void printList(ListRef L){
	if(!isEmpty(L)){
		...
		if(isEmpty(L)){
			...
			moveFirst(L);
			while(L->header->curr != NULL){
			int showData = L->header->curr->data;
